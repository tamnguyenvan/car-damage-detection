...
DATASET_DIR = "/opt/datasets/car-damage-yolo"
SOURCE_DIR_NAMES = ("Car damages dataset", "Car parts dataset")
EXPECTED_CLASS_COUNT = 8

app = modal.App("car-damage-segmentation-training")
output_volume = modal.Volume.from_name("car-damage-segmentation-output-vol", create_if_missing=True)
...
def download_and_convert_dataset() -> None:
    ...
    def find_source_dir():
        ...
        raise RuntimeError(
            f"Could not identify the {EXPECTED_CLASS_COUNT}-class damage source below {source_root}. "
    ...
    if len(class_names) != EXPECTED_CLASS_COUNT:
        raise RuntimeError(
            f"Expected {EXPECTED_CLASS_COUNT} damage classes in {source_dir}, but found "
            f"{len(class_names)}: {class_names}. Verify that the archive contains the damage subset."
        )
    print(f"Using damage source {source_dir} with {len(class_names)} classes: {class_names}")
    ...
    if (output_dir / "data.yaml").exists():
        ...
        if existing_names == class_names:
            print(f"Reusing prepared YOLO damage dataset at {output_dir}.")
            return
    ...
    if written_segments == 0:
        raise RuntimeError("No valid damage polygons were converted; check the annotation schema.")
    if written_images < 700:
        raise RuntimeError(
            f"Expected roughly 814 damage images, but converted only {written_images}. "
    ...
    print(
        f"YOLO damage-segmentation dataset prepared at {DATASET_DIR}: "
...
def train(
    ...
    if "-seg" not in model_name.lower():
        raise ValueError("Damage segmentation training requires a checkpoint such as 'yolo26n-seg.pt'.")

    seed_everything(seed)
    if not os.path.exists(f"{DATASET_DIR}/data.yaml"):
        raise RuntimeError("Baked YOLO damage dataset is missing from the Modal image.")
    resolved_run_name = run_name or _run_name_from_model(model_name)
    print(f"Training {model_name} for damage segmentation as run '{resolved_run_name}' with seed={seed}")
...
def main(
    ...
    resolved_run_name = run_name or _run_name_from_model(model_name)
    print(f"Dispatching damage-segmentation training job for {model_name}...")
    ...
    print(
        ...
        f"/{resolved_run_name}/weights/best.pt ./models/car_damage_yolo26_seg.pt"
    )
